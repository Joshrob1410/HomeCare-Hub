...
export default function ThemeToggle() {
  ...
  // hydrate initial value from cookie
  useEffect(() => {
  ...
  function emitOrbitChanged(nextOrbit: boolean) {
    // same-tab listeners (Dashboard, etc.)
  ...
  // make sure cookie is set *before* refreshing RSC
  ...
  async function apply(nextMode: Mode) {
    ...
    // optional server sync (don’t block UI on this)
    fetch("/api/theme", {
      method: "POST",
  ...
  async function toggle() {
    ...
    // 🔁 re-broadcast on next frame (handles components that remount on refresh)
    requestAnimationFrame(() => emitOrbitChanged(nextOrbit));

    // refresh RSC; also re-broadcast *after* refresh kicks off
    startTransition(() => {
      router.refresh();
      // post-refresh microtask (gives new listeners a chance to attach)
