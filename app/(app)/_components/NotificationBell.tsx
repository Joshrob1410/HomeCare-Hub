...
export default function NotificationBell() {
    ...
    function renderExtra(n: Notif) {
        if (n.kind === 'TRAINING_ASSIGNMENT') {
            const dueRaw = n.payload?.due_by;
            const dueTxt = dueRaw ? new Date(dueRaw).toLocaleDateString() : null;
            const course = n.payload?.course_name as string | undefined;
            return (
        ...
        if (n.kind === 'TRAINING_REMINDER') {
            const course = n.payload?.course_name as string | undefined;
            const bucket = n.payload?.bucket as string | undefined;
            const dueRaw = n.payload?.due_date || n.payload?.next_due_date;
            const dueTxt = dueRaw ? new Date(dueRaw).toLocaleDateString() : null;

            const bucketLabel =
                bucket === 'OVERDUE' ? 'Overdue' :
                    bucket === 'DUE_TODAY' ? 'Due today' :
                        bucket === 'DUE_TOMORROW' ? 'Due tomorrow' :
                            bucket === 'DUE_SOON' ? 'Due soon' : (bucket || 'Reminder');
...
function NotifRow({
    n, busy, markRead, remove, renderExtra,
}: {
    n: Notif;
    busy: boolean;
    markRead: (id: string, yes: boolean) => Promise<void>;
    remove: (id: string) => Promise<void>;
    renderExtra: (n: Notif) => JSX.Element | null;
}) {
    return (
        <div className={`flex items-start gap-3 px-3 py-2.5 rounded-xl border ${n.is_read ? 'bg-white' : 'bg-indigo-50/40 ring-1 ring-indigo-100/70'}`}>
            ...
            <div className="shrink-0 flex flex-col items-end gap-2">
                <div className="flex items-center gap-2">
                    ...
                    {n.link && (
                        <a href={n.link} className="text-xs rounded border px-2 py-1 hover:bg-gray-50">Open</a>
                    )}
                    <button
                        disabled={busy}
                        onClick={() => remove(n.id)}
