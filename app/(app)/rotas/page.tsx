...
type Entry = {
    ...
    hours: number;
    notes: string | null;
    start_time: string | null; // NEW: 'HH:MM:SS' from Postgres time
};
...
function displayName(list: Profile[], id: string) {
    const full = list.find(p => p.user_id === id)?.full_name?.trim();
...
function CalendarGrid({
  ...
  const cells: (number|null)[] = Array.from({length:startDow}, ()=>null).concat(
...
function endTimeFrom(startHHMM: string, hours: number) {
...
function MyRotas({ isAdmin, isCompany, isManager, isStaff }: {
    ...
    const endTimeFrom = (startHHMM: string, hours: number) => {
        const [H, M] = startHHMM.split(':').map(n => parseInt(n, 10));
        if (Number.isNaN(H) || Number.isNaN(M)) return { end: startHHMM, nextDay: false };
        const add = Math.round((hours || 0) * 60);
        let mins = H * 60 + M + add;
    ...
    useEffect(() => {
        (async () => {
            ...
            const list: Home[] = (rpc.data || []) as any[];
    ...
    useEffect(() => {
        (async () => {
            ...
            if (!error) setShiftTypes((data || []) as any[]);
        })();
    ...
    useEffect(() => {
        (async () => {
            ...
            const e = await supabase.from('rota_entries').select('*').eq('rota_id', r.data.id);
    ...
    useEffect(() => {
        (async () => {
            ...
            const rows = ((rs.data as any[]) || []).map(x => ({
                id: x.id,
                rota_id: x.rota_id,
                day_of_month: x.day_of_month,
                user_id: x.user_id,
                shift_type_id: x.shift_type_id,
                hours: x.hours,
                notes: x.notes,
                start_time: x.start_time,
                _home_id: x.rotas?.home_id as string | undefined,
            })) as (EntryWithStart & { _home_id?: string })[];

            setBankEntries(rows);

            // 2) Homes + companies for those rotas
            const homeIds = Array.from(new Set(rows.map(r => r._home_id).filter(Boolean))) as string[];
            if (homeIds.length) {
                const hq = await supabase.from('homes').select('id,name,company_id').in('id', homeIds);
                const map = new Map<string, Home>();
                (hq.data || []).forEach((h: any) => map.set(h.id, h as Home));
                setHomeById(map);

                // 3) Shift types for all involved companies via RPC
                const cids = Array.from(new Set((hq.data || []).map((h: any) => h.company_id)));
                if (cids.length) {
                    let combined: any[] = [];
                    for (const cid of cids) {
                        ...
                        if (data && data.length) combined = combined.concat(data);
                    }
                    // de-dupe by id
                    const seen = new Set<string>();
                    const dedup = combined.filter((st: any) => {
                                ...
                                <div className="space-y-1">
                                    {visible.length === 0 ? (
                                        ...
                                        const titleText = (() => {
                                            ...
                                            const s = hhmm(e.start_time ?? null);
                                            if (!s) return full;
                                            const { end, nextDay } = endTimeFrom(s, e.hours || 0);
                        ...
                        return (
                            <div className="space-y-1">
                                {todays.length === 0 ? (
                                    ...
                                    const code = e.shift_type_id ? codeById.get(e.shift_type_id) : undefined;
                                    const inits = initialsFor(people, e.user_id);
                                    const h = homeById.get(e._home_id || '');
                                    const titleText = (() => {
                                        const full = displayName(people, e.user_id);
                                        const s = hhmm(e.start_time ?? null);
                                        if (!s) return full;
...
// Given a yyyy-mm-01 month date, returns all dates in that month
function daysInMonth(monthISO: string): Date[] {
    const base = new Date(`${monthISO}T00:00:00`);
...
// Week start (Sunday → Saturday week)
// Week start = Sunday
...
function ManageRotas({ isAdmin, isCompany, isManager }: {
    ...
    const [editStart, setEditStart] = useState<string>(''); // NEW: 'HH:MM'
    ...
    useEffect(() => {
        (async () => {
            ...
            if (!isAllHomes) {
                const h = myHomes.find(h => h.id === homeId);
            ...
            const rows = ((rs.data as any[]) || []) as EntryWithStart[];
            setEntries(rows); // not used for calendar when ALL, but useful if needed
        ...
        // include shiftTypes so KPI updates when categories load
    }, [homeId, month, shiftTypes, isAllHomes]);
        ...
        (async () => {
            ...
            if (uniqueIds.length) {
                ...
                setProfiles(prev => {
                    ...
                    (prof.data || []).forEach((p: any) => map.set(p.user_id, p.full_name));
            ...
            const homeIds = (homeOnly.data || []).map((x: any) => x.user_id);
    ...
    async function ensureRota(): Promise<Rota | undefined> {
        ...
        const ins = await supabase.from('rotas').insert({
            home_id: homeId, month_date: month, status: 'DRAFT', created_by: u.user?.id
        }).select('*').single();
        if (ins.error) { alert(ins.error.message); return; }
        setRota(ins.data as any);
        return ins.data as Rota;
    }

    async function makeLive() {
        if (!rota) { alert('No rota to publish. Add an entry first.'); return; }
        const { data, error } = await supabase.rpc('publish_rota', { p_rota_id: rota.id });
    ...
    async function setDraft() {
        if (!rota) return;
        const { data, error } = await supabase.rpc('unpublish_rota', { p_rota_id: rota.id });
        if (error) { alert(error.message); return; }
    ...
    function openEditor(day: number, entry?: EntryWithStart) {
        ...
        if (entry) {
            ...
            setEditStart(hhmm(entry.start_time ?? null) || '');
        } else {
    ...
    async function saveEditor() {
        ...
        const payload = {
            ...
            work_home_id: rr.home_id, // ✅ required by your RLS policies
        };
        ...
        if (!editEntryId) {
            ...
            setEntries(prev => {
                ...
                if (i === -1) return [...prev, ins.data as any];
                const next = prev.slice();
                next[i] = ins.data as any;
                return next;
            });
        } else {
            const upd = await supabase
                ...
                .update({
                    ...
                    work_home_id: rr.home_id, // ✅ ensure/backfill for RLS on UPDATE too
                })
                .eq('id', editEntryId)
                .select('*')
                .single();
            ...
            setEntries(prev => prev.map(e => (e.id === editEntryId ? (upd.data as any) : e)));
        }
    ...
    async function deleteEntry(id: string) {
        ...
        if (error) {
            ...
            setEntries((refreshed.data || []) as any[]);
        }
    }
    ...
    function toUKDate(isoYmd: string) {
                                    ...
                                    <div>
                                        ...
                                        {editStart && (
                                            <p className="mt-1 text-xs text-gray-600">
                                                Ends at {
                                                    (() => {
                                                        const { end, nextDay } = endTimeFrom(editStart, editHours || 0);
...
function RotaSettings({ isAdmin }:{ isAdmin:boolean }) {
  ...
  async function addType(e:React.FormEvent) {
    ...
    setList([...(list as any), ins.data as any]);
  ...
  async function toggleActive(id:string, is_active:boolean) {
    ...
    if (!upd.error) setList(list.map(s=> s.id===id ? upd.data as any : s));
  }

  async function saveRow(row: ShiftType, patch: Partial<ShiftType>) {
    const upd = await supabase.from('shift_types').update(patch as any).eq('id', row.id).select('*').single();
    if (upd.error) { alert(upd.error.message); return; }
    setList(list.map(s=> s.id===row.id ? upd.data as any : s));
  }
      ...
      <section className="rounded-xl border bg-white shadow-sm ring-1 ring-gray-50 p-4 space-y-3">
        ...
        {isAdmin && (
          <div>
            ...
            <select className="w-full max-w-sm border rounded-lg px-3 py-2" value={companyId} onChange={e=>setCompanyId(e.target.value)}>
        ...
        <form onSubmit={addType} className="grid grid-cols-1 sm:grid-cols-5 gap-3">
          <div>
            ...
            <input className="w-full border rounded-lg px-3 py-2" value={code} onChange={e=>setCode(e.target.value)} required />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm mb-1">Label</label>
            <input className="w-full border rounded-lg px-3 py-2" value={label} onChange={e=>setLabel(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm mb-1">Default hours</label>
            <input type="number" min={0} step="0.25" className="w-full border rounded-lg px-3 py-2" value={defHours} onChange={e=>setDefHours(Number(e.target.value))} required />
          </div>
          <div>
          ...
          <div className="sm:col-span-5">
            <button className="rounded-lg border px-3 py-2 text-sm hover:bg-gray-50" disabled={!companyId}>Add</button>
        ...
        <div className="max-h-[28rem] overflow-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              ...
              {(!list || list.length===0) && (
                <tr><td className="p-2 text-sm text-gray-500" colSpan={6}>No shift types yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
  ...
  return (
    <tr className="border-t align-top">
      <td className="p-2 font-mono">
        {editing ? <input className="border rounded px-2 py-1 text-sm w-full" value={code} onChange={e=>setCode(e.target.value)} /> : item.code}
      </td>
      <td className="p-2">
        {editing ? <input className="border rounded px-2 py-1 text-sm w-full" value={label} onChange={e=>setLabel(e.target.value)} /> : item.label}
      </td>
      <td className="p-2">
        {editing ? <input type="number" min={0} step="0.25" className="border rounded px-2 py-1 text-sm w-full" value={hours} onChange={e=>setHours(Number(e.target.value))} /> : item.default_hours}
      </td>
      <td className="p-2">
        {editing ? (
          <select className="border rounded px-2 py-1 text-sm w-full" value={kind} onChange={e=>setKind(e.target.value)}>
            {SHIFT_KINDS.map(k => <option key={k.value} value={k.value}>{k.label}</option>)}
          </select>
        ) : (
          <span className="text-gray-700">{SHIFT_KINDS.find(k=>k.value===(item.kind||''))?.label || '(none)'}</span>
        )}
      </td>
      <td className="p-2">
      ...
      <td className="p-2">
        {!editing ? (
          <div className="flex gap-2">
            <button onClick={()=>setEditing(true)} className="rounded border px-2 py-1 text-xs hover:bg-gray-50">Edit</button>
            <button onClick={onDelete} className="rounded border px-2 py-1 text-xs hover:bg-gray-50">Delete</button>
          </div>
        ) : (
          <div className="flex gap-2">
            <button disabled={busy} onClick={save} className="rounded border px-2 py-1 text-xs hover:bg-gray-50 disabled:opacity-60">{busy?'Saving…':'Save'}</button>
            <button
              disabled={busy}
              onClick={()=>{ setEditing(false); setCode(item.code); setLabel(item.label); setHours(item.default_hours); setKind(item.kind || ''); }}
