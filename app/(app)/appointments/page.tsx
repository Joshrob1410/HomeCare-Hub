...
export default function Page() {
    ...
    // orbit-aware inline banner instead of alert popups
            ...
            if (lvl === "1_ADMIN") {
                ...
                if (hid) {
                    ...
                    if (h) {
                        ...
                        const { data: c } = await supabase.from("companies").select("id,name").eq("id", home.company_id).single();
    ...
    const loadAppointments = useCallback(async () => {
        ...
        const [{ data: ap }, { data: ex }] = await Promise.all([
            supabase.from("appointments").select("*").eq("home_id", selectedHomeId).lte("start_date", to),
            supabase.from("appointment_exceptions").select("*").gte("skip_date", from).lte("skip_date", to),
        ]);
    ...
    async function deleteOccurrence(a: Appointment) {
        ...
        if (ins.error) {
            pushNotice(ins.error.message, "error");
            return;
        }
        setShowForm(false);
        await loadAppointments();
    ...
    /** Trigger reminders now (RPC; SQL handles de-duplication) */
    ...
    if (view.status === "loading") {
        return (
            <div className="p-4" style={{ color: "var(--sub)" }}>
                Loading…
            </div>
        );
    }
    ...
    return (
        <div className="p-4 md:p-6 space-y-4" style={{ color: "var(--ink)" }}>
            <div className="flex flex-wrap items-end gap-3">
                {/* Orbit-aware inline banner */}
                {notice && (
                    <div
                        className={[
                            ...
                            // Orbit overrides
                            notice.type === "error"
                                ? "[data-orbit='1']:bg-rose-500/10 [data-orbit='1']:border-rose-400/25 [data-orbit='1']:text-rose-200"
                                : notice.type === "success"
                                    ? "[data-orbit='1']:bg-emerald-500/10 [data-orbit='1']:border-emerald-400/25 [data-orbit='1']:text-emerald-200"
                                    : "[data-orbit='1']:bg-indigo-500/10 [data-orbit='1']:border-indigo-400/25 [data-orbit='1']:text-indigo-200",
                        ].join(" ")}
                    >
                        {notice.text}
                    </div>
                ...
                <h1 className="text-2xl font-semibold mr-auto" style={{ color: "var(--ink)" }}>
                    Appointments
                </h1>
                ...
                <div className="flex items-center gap-2">
                    <button
                        className="h-9 w-9 inline-flex items-center justify-center rounded-md ring-1 transition active:scale-[0.98]"
                    ...
                    <div className="text-sm min-w-[140px] text-center" style={{ color: "var(--sub)" }}>
                    ...
                    <button
                        className="inline-flex items-center rounded-md ring-1 px-2 py-2 text-xs transition active:scale-[0.98]"
                        style={{ background: "var(--nav-item-bg)", borderColor: "var(--ring)", color: "var(--ink)" }}
                ...
                <div className="flex items-center gap-2">
                    <button
                        className="inline-flex items-center rounded-md text-sm px-3 py-2 shadow-sm ring-1 transition"
                        style={{
            ...
            {/* Calendar grid */}
            <div className="rounded-xl overflow-hidden ring-1" style={{ background: "var(--card-grad)", borderColor: "var(--ring)" }}>
                <div className="grid grid-cols-7 text-xs" style={{ background: "var(--nav-item-bg)", borderBottom: "1px solid var(--ring)" }}>
                    {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
                        <div key={d} className="px-2 py-2 font-medium" style={{ color: "var(--sub)", borderRight: "1px solid var(--ring)" }}>
                            {d}
                        </div>
                    ))}
                </div>

                {/* use 1px gaps by making the wrapper the "gridline" color */}
                <div className="grid grid-cols-7 gap-px" style={{ background: "var(--ring)" }}>
                    {days.map(({ iso, inMonth }) => {
                        const list = eventsByDay.get(iso) || [];
                        return (
                            <div
                                key={iso}
                                className="p-2 min-h-[96px]"
                                style={{
                                    background: "var(--nav-item-bg)",
                            ...
                            >
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    {inMonth && view.selectedHomeId && (
                                        <button
                                            className="text-[11px] px-1.5 py-0.5 rounded ring-1 transition"
                                ...
                                <div className="space-y-1">
                                    {list.map((a) => (
                                        <button
                                            ...
                                            className="block w-full text-left text-[12px] rounded ring-1 transition"
                                            style={{
                                                background: "var(--nav-item-bg)",
                                                borderColor: "var(--ring)",
                                                color: "var(--ink)",
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <span className="text-[10px]" style={{ color: "var(--brand-link)" }}>
                                                    {a.recurrence === "NEVER" ? "" : a.recurrence.toLowerCase()}
                                                </span>
                                            </div>
                ...
                {loading && (
                    <div className="p-3 text-sm" style={{ color: "var(--sub)" }}>
                        Loading…
                    </div>
                )}
            </div>
            ...
            {/* Add/Edit modal */}
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-3" style={{ background: "rgba(0,0,0,0.30)" }}>
                    {/* .modal-panel gets solid bg in Orbit via global CSS below */}
                    <div
                        className="modal-panel rounded-xl p-4 w-[560px] max-w-[92vw] shadow-lg ring-1"
                        style={{ background: "var(--nav-item-bg)", borderColor: "var(--ring)", color: "var(--ink)" }}
                    >
                        ...
                        <form onSubmit={saveForm} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div className="sm:col-span-2">
                                ...
                                <input
                                    type="text"
                                    className="rounded px-2 py-2 text-sm w-full ring-1"
                                    style={{ background: "var(--nav-item-bg)", color: "var(--ink)", borderColor: "var(--ring)" }}
                                    value={formDesc}
                            ...
                            <div className="sm:col-span-2 flex items-center gap-3 mt-1">
                                ...
                                <button
                                    ...
                                    style={{
                                        background: formRem ? "rgba(16,185,129,0.65)" : "var(--ring-strong)",
                                    }}
                                >
                                    <span
                                        className="inline-block h-6 w-6 transform rounded-full bg-white shadow transition-transform"
                                        style={{ transform: formRem ? "translateX(24px)" : "translateX(4px)" }}
                                    />
                                </button>
                                ...
                                <span className="text-xs" style={{ color: "var(--sub)" }}>
                                    Sends to Staff & Managers: 7 days before, the day before, and on the day.
                                </span>
                            </div>

                            {/* Danger zone (inline, no popups) */}
                            {editing && (
                                <div className="sm:col-span-2 mt-2 rounded-lg p-3 ring-1 bg-rose-50 border-rose-200 text-rose-800 [data-orbit='1']:bg-rose-500/10 [data-orbit='1']:ring-rose-400/25 [data-orbit='1']:text-rose-200">
                                        ...
                                        {occurrenceISO ? (
                                            confirmRow === "this" ? (
                                                <div className="flex gap-2">
                                                    <button type="button" className="px-3 py-1.5 text-sm rounded-md text-white" style={{ background: "#DC2626" }} onClick={() => deleteOccurrence(editing!)}>
                                            ...
                                            <span className="text-xs">Open from a calendar day to target a single occurrence.</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-2 py-1">
                                        <div className="text-sm">
                                            Delete the <span className="font-medium">entire series</span> (all future occurrences).
                                        </div>
                                        {confirmRow === "series" ? (
                                            <div className="flex gap-2">
                                                <button type="button" className="px-3 py-1.5 text-sm rounded-md text-white" style={{ background: "#DC2626" }} onClick={() => deleteSeries(editing!)}>
                            ...
                            <div className="sm:col-span-2 flex justify-end gap-2 mt-2">
                                <button type="button" className="px-3 py-2 text-sm rounded-md ring-1" style={{ background: "var(--nav-item-bg)", borderColor: "var(--ring)", color: "var(--ink)" }} onClick={closeForm}>
                                    Close
                                </button>
                                <button
            ...
            {/* Orbit-only control + modal fixes (scoped to this page) */}
            <style jsx global>{`
        /* Make native popovers dark in Orbit, and ensure closed controls aren't washed out */
        [data-orbit="1"] select,
        ...
        /* Options inside opened dropdown menu */
        [data-orbit="1"] select option {
          color: var(--ink);
          background-color: #0b1221; /* solid so options don't look transparent */
        }
        @-moz-document url-prefix() {
          [data-orbit="1"] select option {
            background-color: #0b1221; /* ensure Firefox list is solid dark */
          }
        }
        /* Remove unintended fade */
        [data-orbit="1"] select:where(:not(:disabled)) {
          opacity: 1;
        }
        /* Solid modal surface in Orbit so it doesn't blend with the page gradient */
        [data-orbit="1"] .modal-panel {
          background-color: #0b1221 !important; /* deep navy, fully opaque */
        }
      `}</style>
        </div>
    );
